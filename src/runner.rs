...
///
/// When local file access is enabled (`git.clone` is true), returns
/// `Some(instructions)` containing the workspace directory path and an
/// explicit `cd` directive. When `git.clone` is false (no local file
/// access), returns `None` — the `instructions` field is omitted from
/// the API request entirely, since the step name is already passed as
/// the prompt (`input`).
