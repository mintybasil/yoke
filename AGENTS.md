...
src/
  ...
  github_api.rs — GitHub REST API client (GitHubClient, Webhook struct, list_webhooks with pagination, error types)
...
- **GitHub API client** (`src/github_api.rs`): `GitHubClient` struct wraps `reqwest::Client` with Bearer token authentication. Exposes `list_webhooks(owner, repo)` that handles GitHub API pagination transparently (follows `Link` rel="next" headers). Error mapping: HTTP 401→`Unauthorized`, 404→`NotFound`, 403→`RateLimited`, others→`ApiError`. Unit tests use `mockito` for HTTP mocking and cover: success (single page), empty list, pagination (two pages), auth failure (401), not found (404), rate limiting (403).
