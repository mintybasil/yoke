...
src/
  ...
  dispatcher.rs — Concurrency control (Dispatcher + Semaphore), deduplication (DedupSets, SharedDedupSets), persistence, workspace directory management, and git orchestration (base repo + worktree for review events)
...
- **Workspace directory** (`src/dispatcher.rs`): `workspace_dir(workdir, owner, repo, event_id)` constructs the per-event workspace path `{workdir}/{owner}/{repo}/{event_id}/` per the architecture design doc (Section 11). The dispatcher creates this directory before spawning the workflow task. Git orchestration is **config-driven**: when any matching workflow has `[git] clone = true`, the dispatcher calls `ensure_base_repo()` to clone the repository on first event or pull the latest on subsequent events to a shared base clone at `{workdir}/{owner}/{repo}/repo/`; when any matching workflow has `[git] worktree = true`, the dispatcher calls `git::create_worktree()` to initialize the event workspace as a git worktree. This is not limited to review triggers — any workflow type can opt into git features via its `[git]` config section. Both `clone` and `worktree` default to `false` (opt-in). Branch resolution: the dispatcher prefers `TriggerEvent.branch` (from the webhook payload), falling back to the workflow's `git.default_branch`. If no branch is available when a worktree is requested, the event is permanently failed. The `base_repo_path()` function returns the canonical path for the shared base clone. The actual prompt and log files are written by `WorkflowRunner::execute_step()` using real API request/response data.
