...
src/
  ...
  catch_up.rs  — Catch-up (event replay) module: replays missed webhook events on startup using GitHub/GitLab delivery APIs and watermark-based dedup
  ...
  git.rs        — Git repository operations (shallow clone, auth callbacks, branch sanitization, dirty-check)
  github_api.rs — GitHub REST API client (GitHubClient, Webhook, WebhookConfig, WebhookOrchestrationSummary, WebhookDelivery, WebhookDeliveryDetail, DeliveryRequest, GitHubError with ValidationError variant, list/create/update/delete webhooks, list_deliveries/get_delivery/redeliver_delivery, find_webhook_by_url, ensure_webhook, orchestrate_webhooks, map_status_with_body for response body capture in errors)
...
- **CRITICAL: `allowed_users` is a SECURITY BOUNDARY, not an event-content filter**: `allowed_users` exists to prevent prompt injection attacks by restricting which users can **invoke** a workflow. It must NEVER be conflated with `assigned_to` or `mentioned_user`. Those are event-content filters (they describe who the event is ABOUT). `allowed_users` is an authorization check (it describes who is PERMITTED TO TRIGGER the workflow). The **actor** checked against `allowed_users` is the user who **performed the action** that created the webhook event (extracted from `payload.sender.login` on GitHub, `payload.user.username` on GitLab, except `payload.review.user.login` for GitHub `pull_request_review` events with a fallback to `payload.sender.login`) — NOT the assignee or mentioned user. For example, when an issue is assigned to alice by bob, the actor is bob (who did the assigning), not alice (who was assigned). `Workflow::validate()` enforces that every workflow has a non-empty `allowed_users` list; omitting it would allow any user to trigger the workflow. The dispatcher extracts the actor from the webhook payload at receipt time and rejects events from unauthorized actors. See the "Trigger Authorization" section in `docs/Architecture Design.md` for the full actor mapping table.
...
- **Environment variable validation**: `validate_env_vars(platform)` checks required env vars at startup. `HERMES_API_KEY` is always required; `GITHUB_TOKEN` is required when `platform = "github"`, `GITLAB_TOKEN` when `platform = "gitlab"`. `WEBHOOK_SECRET` is always required — it is the sole source of the webhook secret, not an override of a config field. Returns `ConfigError::EnvVar` with a descriptive message.
- **`WEBHOOK_SECRET` env var**: The `WEBHOOK_SECRET` env var is required and is the only way to provide the webhook secret (HMAC key for GitHub, token for GitLab). There is no config file fallback. If `WEBHOOK_SECRET` is not set, the process exits with an error at startup.
- **GitHub API client** (`src/webhook/github_api.rs`): `GitHubClient` struct wraps `reqwest::Client` with Bearer token authentication. Public API: `list_webhooks(owner, repo)` with transparent pagination (follows `Link` rel="next" headers), `create_webhook(owner, repo, config)` (POST to `/repos/{owner}/{repo}/hooks`), `update_webhook(owner, repo, webhook_id, config)` (PATCH to `/repos/{owner}/{repo}/hooks/{id}`), `delete_webhook(owner, repo, webhook_id)` (DELETE to `/repos/{owner}/{repo}/hooks/{id}`). Delivery API: `list_deliveries(owner, repo, hook_id)` with transparent pagination (follows `Link` rel="next" headers, returns `Vec<WebhookDelivery>`), `get_delivery(owner, repo, hook_id, delivery_id)` (returns `WebhookDeliveryDetail`), `redeliver_delivery(owner, repo, hook_id, delivery_id)` (POST to `/repos/{owner}/{repo}/hooks/{hook_id}/deliveries/{delivery_id}/attempts`, expects 202 Accepted). Delivery data models: `WebhookDelivery` (summary with `id`, `guid`, `event`, `action`, `delivered_at` as `DateTime<Utc>`, `status`), `WebhookDeliveryDetail` (full detail adding `response_url`, `response_status`, `response_code`, `duration`, `request_payload`, `response_payload`), `DeliveryRequest` (redelivery request with `action`, `timestamp` as `DateTime<Utc>`). `create_webhook` and `update_webhook` validate that `config.events` is non-empty before making the API call, returning `ValidationError` if empty. `WebhookConfig` struct (`url`, `secret`, `events`) is passed to create/update. `find_webhook_by_url(webhooks, url)` searches a webhook list by URL for idempotency checks. `ensure_webhook(owner, repo, config)` is an idempotent orchestrator that lists existing webhooks, finds by URL, and either updates the existing webhook or creates a new one — returning a `WebhookOrchestrationSummary`. `orchestrate_webhooks(repos, config)` iterates over multiple `(owner, repo)` pairs, calling `ensure_webhook` for each and aggregating results into a single summary with `created`, `updated`, and `skipped` counts. Error mapping via `map_status_with_body`: HTTP 401→`Unauthorized`, 404→`NotFound`, 403→`RateLimited`; all other non-success statuses include the response body in the `ApiError` message (format: `"{status} - {body}"`) for easier debugging. `ValidationError` variant is used for client-side validation failures (e.g., empty events list). Unit tests use `mockito` for HTTP mocking and cover: list (success, empty, pagination, 401, 404, 403, 422 with body), create (success 201, empty events validation, 422 with body), update (success 200, empty events validation, not found 404), delete (success 204, not found 404, 422 with body), `find_webhook_by_url` helper, `ensure_webhook` (creates new, updates existing), `orchestrate_webhooks` (multi-repo with partial failures), list_deliveries (success, empty, pagination, not found), get_delivery (success, unauthorized), redeliver_delivery (success, api error
- **GitLab API client** (`src/webhook/gitlab_api.rs`): `GitLabClient` struct wraps `reqwest::Client` with Private-Token authentication. Public API: `list_webhooks(project_id)` with transparent pagination (follows `Link` rel="next" headers), `create_webhook(project_id, config)` (POST, returns 201 Created), `update_webhook(project_id, webhook_id, config)` (PUT, returns 200 OK), `delete_webhook(project_id, webhook_id)` (DELETE, returns 204 No Content), `list_project_events(project_id, after)` (GET `/projects/{id}/events?after={timestamp}`, transparent pagination). `WebhookConfig` struct (`url`, `token`, `push_disabled`, `active`, `events`) is passed to create/update; `Option` fields use `skip_serializing_if` to omit nulls from JSON. `ProjectEvent` struct represents a single event from the GitLab Projects Events API; `ProjectEvent::try_into_trigger_event(repo_path)` maps events to Yoke `TriggerEvent` (supports Issue opened/updated → `GitlabIssueAssigned`, Issue comment → `GitlabIssueMention`, MR comment → `GitlabMergeRequestReview`; push events are skipped). `ProjectPushData` carries push-specific fields (`commit_sha`, `branch`, `commit_count`). Supports self-hosted GitLab via configurable `base_url` (default: `https://gitlab.com/api/v4`). Also provides `find_webhook_by_url` for idempotency checks. Error mapping: HTTP 401→`Unauthorized`, 404→`NotFound`, others→`ApiError`. Unit tests use `mockito` for HTTP mocking and cover: list (success, empty, pagination, 401, 404, 500), create (success 201, 401), update (success 200, 404, 401), delete (success 204, 404, 401), `list_project_events` (success, pagination, 401, 404), `WebhookConfig` serialization (full, minimal), `find_webhook_by_url` (found, not found, empty), client construction (default/custom base URL), and `ProjectEvent` mapping (Issue opened/updated, Issue comment, MR comment, unsupported actions, variable extraction).
...
- **Graceful shutdown flow**: When the signal handler sends `true` on the watch channel: (1) the axum HTTP server's `with_graceful_shutdown` future detects the change and stops accepting new connections, (2) the dispatcher's `run_with_drain` loop stops consuming from the mpsc channel, (3) the dispatcher waits up to `config.runtime.drain_timeout_secs` (default: 30s) for in-flight workflows to complete (checking `active_count` every 100ms), (4) state is persisted via `Dispatcher::persist_state()` which writes `completed.json`, `failed.json`, and `watermark.json` atomically, (5) the process exits. A second SIGINT/SIGTERM during the drain phase forces an immediate `process::exit(1)`.
...
- **Catch-up / event replay** (`src/catch_up.rs`): On startup, Yoke replays webhook events that were missed during downtime. `run_catch_up(config, server_config, watermark_store, tx)` is called in `run_server()` before the HTTP listener starts. It reads the watermark timestamp per repo from `WatermarkStore`, queries the platform's event delivery API (GitHub: `list_deliveries` + `get_delivery`; GitLab: `list_project_events`) for events newer than the watermark, and dispatches them via the same mpsc channel used for live webhooks. **First-run behavior**: If the watermark store is empty (no watermarks for any repo), catch-up is skipped entirely at the top level before iterating repos — without a baseline timestamp, replaying all events within the `catch_up_max_age_hours` window could trigger unexpected behaviour. If some repos have watermarks but others don't (e.g. a new repo was added to config after a previous run), the per-repo check still skips individual repos with no watermark. Catch-up resumes on subsequent starts once a watermark has been established. **Dedup interaction**: Because catch-up replays use the same canonical `event_id` format as live webhooks (e.g. `owner/repo/issue-42`), the existing three-set dedup mechanism (`in_flight`, `completed`, `permanently_failed`) naturally prevents duplicate processing when a live webhook and a replayed event overlap. A replayed event whose dedup key is already in any set is silently skipped — no workflow is spawned, no state change occurs. This means: (1) concurrent live + replayed events for the same logical event produce only one workflow run; (2) events already completed in a previous run are skipped; (3) events that permanently failed in a previous run are also skipped (they are not retried). The `delivery_id` field (GitHub's `X-GitHub-Delivery` GUID) is stored on `TriggerEvent` for watermark tracking only — it is NOT used for dedup. Configuration: `catch_up_enabled` (default: `true`) and `catch_up_max_age_hours` (default: `24`) in the `[server]` section. GitHub catch-up finds the webhook matching the configured `webhook_host` URL, lists deliveries, fetches full delivery bodies via `get_delivery`, decodes `payload_base64`, and replays through the existing `parse_github_event` + `map_to_trigger_event` pipeline. The `replay_github_delivery` function also extracts `mentioned_user` from the comment body (via `extract_mention`) and inserts it into the variables map for `IssueComment` and `PullRequestReviewComment` events, ensuring that template rendering has the same variables available as the live webhook path. GitLab catch-up uses `list_project_events(project_id, after=watermark_timestamp)` and replays through `ProjectEvent::try_into_trigger_event`. Both use watermark-based time filtering and dedup to prevent duplicate processing. `CatchUpSummary` tracks replayed/skipped/errored counts per repo and displays a human-readable summary via `Display`.
...
- **Git operations module**: `src/git.rs` provides git repository management for the dispatcher pipeline. Public API: `sanitize_branch_name()`, `build_clone_url()`, `embed_token_in_url()`, `GitAuth` (credential callbacks for GitHub/GitLab tokens), `clone_repo()`, `shallow_clone()`, `has_uncommitted_changes()`. Token-based auth is wired through `GitAuth` callbacks — GitHub tokens go in the `x-access-token` header, GitLab tokens in the `PRIVATE-TOKEN` header. Branch names are sanitized to `[a-zA-Z0-9._-]` with whitespace collapsed to `-`. `shallow_clone(url, branch, target_dir)` performs a per-event `git clone --depth=1 --branch <branch>` into the workspace directory, removing any existing directory first. `embed_token_in_url()` injects the auth token into the clone URL for CLI-based shallow clone authentication. The `worktree` feature has been removed — `GitConfig` rejects `worktree = true` at deserialization time with a migration error directing users to `clone = true`.

- **GitHub webhook handler**: `src/webhook/github.rs` provides HMAC-SHA256 signature verification (`verify_github_signature`), JSON payload parsing (`parse_github_event`), and event-to-trigger mapping (`map_to_trigger_event`). The `handle_github_webhook` function orchestrates the full flow. `GitHubEvent` carries a `repository` field (`RepositoryDetails` with `full_name`) used for `repo_path` extraction. GitHub uses the `issue_comment` webhook event for both issue and PR comments (since PRs are issues in the API). When the `pull_request` field is present inside the `issue` object in the `issue_comment` payload (i.e., `issue.pull_request` is `Some`), the comment is on a PR and maps to `GithubPullRequestCommentMention`; otherwise it maps to `GithubIssueCommentMention`. The PR number for `pr_number` comes from `issue.number` (which is the PR number when the issue is a PR). Trigger-specific variables are extracted per event type: `github_issue_assigned` → `issue_number`, `assignee`, `issue_title`, `issue_body`; `github_issue_comment_mention` → `issue_number`, `comment_id`, `comment_body`, `mentioned_user`; `github_pull_request_review` → `pr_number`, `review_id`, `review_body`; `github_pull_request_comment_mention` → `pr_number`, `review_id`, `comment_id`, `comment_body`, `mentioned_user` (when triggered via `issue_comment` on a PR, `review_id` is not available and only `pr_number`, `comment_id`, `comment_body`, and `mentioned_user` are populated).
...
- **`TriggerEvent` struct**: Shared webhook result type in `webhook/mod.rs` with `trigger_type: TriggerType`, `repo_path`, `event_id`, `actor: String`, `variables: HashMap<String, String>`, and `delivery_id: Option<String>` fields. The `event_id` field holds the canonical event identifier as defined per trigger type in Appendix A of the architecture design — e.g. `issue-42`, `pr-7-review-999`, `issue-42-comment-12345`. This canonical form is constructed by the webhook handlers and must be used as-is throughout the dispatcher (dedup keys, workspace directories, template variables, log fields) without any stripping or transformation. The `actor` field holds the username of the user who performed the action (`sender.login` for GitHub, `user.username` for GitLab) — this is checked against the workflow's `allowed_users` list by the dispatcher. The `variables` map carries trigger-specific template variables extracted from the webhook payload (e.g., `issue_number`, `comment_id` for GitHub; `issue_iid`, `note_id` for GitLab). These are merged with global variables (`owner`, `repo`, `output_dir`, `event_id`, `repo_path`) in the dispatcher before template rendering. The `delivery_id` field carries the platform-specific delivery ID for watermark tracking — GitHub populates it with the `X-GitHub-Delivery` header UUID; GitLab currently sets it to `None`. This field is dispatcher metadata, not a template variable. Sent to the dispatcher via the mpsc channel in `WebhookHandler`.
- **`WebhookHandler` struct**: Holds `platform`, `secret`, and `sender: mpsc::Sender<TriggerEvent>`. Created in `run_server()` with a bounded channel and passed to `AppState`. Derives `Clone`. The `handle_webhook` method accepts a `delivery_id: Option<String>` parameter for the platform-specific delivery identifier (GitHub: `X-GitHub-Delivery` header UUID; GitLab: `None`) which is assigned to `TriggerEvent.delivery_id` before dispatch. This field is used by the dispatcher for watermark tracking, not for prompt templating.
...
| `git2` | libgit2 bindings for git repository operations (clone, auth callbacks) |
...
- Persistence: completed and failed events are written to `completed.json` / `failed.json`; watermarks to `watermark.json`
