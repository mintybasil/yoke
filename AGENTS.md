...
- **allowed_users validation**: `Workflow::validate()` enforces that every workflow has a non-empty `allowed_users` list in its `[trigger]` section. This prevents accidental open access — without it, any user could trigger the workflow. `TriggerType::requires_allowed_users()` returns true for all trigger types, and `TriggerType::actor()` extracts the relevant user (assignee, mentioned user, or reviewer) from runtime payload fields so the dispatcher can reject events from unauthorized users.
