...
- **`WebhookHandler` struct**: Holds `platform`, `secret`, and `sender: mpsc::Sender<TriggerEvent>`. Created in `run_server()` with a bounded channel and passed to `AppState`. Derives `Clone`. The `handle_webhook` method accepts an optional `extra_variables: HashMap<String, String>` parameter for injecting platform-specific headers (e.g., `github_delivery_id` from `X-GitHub-Delivery`) into the `TriggerEvent.variables` map before dispatch.
