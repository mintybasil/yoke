...
- **Dispatcher and concurrency control** (`src/dispatcher.rs`): The `Dispatcher` struct wraps `SharedDedupSets`, `WatermarkStore` (for tracking last-processed events per repository), and an optional `tokio::Semaphore` to coordinate concurrency limiting, deduplication, and watermark tracking for webhook event processing. `find_matching_workflows` selects workflows whose trigger label matches the event **and** whose `allowed_users` list includes the event's actor — this is the authorization check that enforces the `allowed_users` SECURITY BOUNDARY. The dispatcher reads `HERMES_API_KEY` once before the workflow loop (validation ensures every workflow has at least one step, so the key is always needed). Deferred allocation (#168): `find_matching_workflows()` is called **before** acquiring any resources — events with no matching workflow skip semaphore permits, in-flight marks, and workspace directory creation entirely. `handle_dispatch` is the per-event entry point; it acquires a semaphore permit (if concurrency is limited), marks the event in-flight, creates a workspace directory, then spawns a tokio task that runs the matching workflows sequentially. The task also updates the watermark for the repository on successful completion. The `run_with_drain` method drives the main loop, consuming `DispatchMessage`s from the mpsc channel and forwarding to `handle_dispatch`. On shutdown (watch channel → true), `run_with_drain` stops consuming and waits for in-flight workflows to complete (up to `drain_timeout`).
...
- Permits released: semaphore permits return after task completion, and `active_count` returns to 0 after all `handle_dispatch` tasks complete (both limited and unlimited concurrency)
